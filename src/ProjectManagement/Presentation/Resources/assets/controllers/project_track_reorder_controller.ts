...
export default class extends Controller<HTMLElement> {
    ...
    dragOver(event: DragEvent): void {
        ...
        const hoveredItem = event.target.closest("[data-project-track-reorder-target='item']");
    ...
    private async submitOrder(): Promise<void> {
        ...
        const orderedTrackUuids = this.itemTargets
            .map((item) => item.dataset.itemUuid)
            .filter((trackUuid): trackUuid is string => trackUuid !== undefined && trackUuid !== "");

        this.inputTarget.value = JSON.stringify(orderedTrackUuids);
